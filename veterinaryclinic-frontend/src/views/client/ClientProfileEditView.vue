...
import { getClientProfile, updateClientProfile } from '../../api/services/clientService'
...
const userInitial = computed(() => form.name?.charAt(0).toUpperCase() || 'U')
...
async function handleSubmit() {
  try {
    ...
    saving.value = true
    await updateClientProfile({ name: form.name.trim(), phone: form.phone.trim(), address: form.address.trim() })
    successMessage.value = 'Profilul a fost actualizat cu succes.'
    setTimeout(() => router.push({ name: 'client-profile' }), 1200)
  } catch {
...
<template>
  <div class="page-wrapper">
    <div class="page-nav">
      <button class="back-btn" @click="router.push({ name: 'client-profile' })">← Înapoi la profil</button>
    </div>
    ...
    <div class="edit-card">
      ...
      <form v-else class="edit-form" @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          ...
          <textarea id="address" v-model="form.address" rows="3" placeholder="Introdu adresa" />
        </div>

        <div class="form-actions">
          <button type="button" class="cancel-btn" @click="router.push({ name: 'client-profile' })">Anulează</button>
...
.avatar-img { width: 100px; height: 100px; border-radius: 50%; object-fit: cover; border: 4px solid #fce7f3; box-shadow: 0 6px 20px rgba(190,24,93,0.15); }
...
.avatar-edit-btn { position: absolute; bottom: 4px; right: 4px; width: 28px; height: 28px; border-radius: 50%; border: 2px solid white; background: #be185d; color: white; font-size: 0.7rem; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 6px rgba(0,0,0,0.2); }
.avatar-edit-btn:hover { background: #9d174d; }
.hidden-input { display: none; }
.avatar-btns { display: flex; gap: 8px; }
.upload-btn { border: 1px solid #fbcfe8; border-radius: 10px; padding: 6px 14px; font-size: 0.83rem; font-weight: 600; cursor: pointer; background: #fdf2f8; color: #be185d; }
.upload-btn:hover { background: #fce7f3; }
...
.form-group input:focus, .form-group textarea:focus { outline: none; border-color: #ec4899; box-shadow: 0 0 0 3px rgba(236,72,153,0.12); }
...
.save-btn { border: none; border-radius: 12px; padding: 10px 22px; background: linear-gradient(135deg, #be185d, #9d174d); color: white; font-weight: 700; cursor: pointer; font-size: 0.95rem; box-shadow: 0 4px 14px rgba(190,24,93,0.25); }
.save-btn:hover { transform: translateY(-1px); box-shadow: 0 8px 20px rgba(190,24,93,0.3); }
