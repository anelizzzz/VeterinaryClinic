...
import { getLabResultsByPetId, TestType, type LabResultResponseDto } from '../../api/services/labResultService'
...
async function handleGenerateAiDiagnosis(result: LabResultResponseDto) {
  try {
    ...
    labResults.push(`Tip test: ${testTypeLabel(result.testType)}`)
  ...
  <div class="page-wrapper">
    <div class="profile-card">
      <div class="page-header">
        ...
        <p>Vizualizează și analizează rezultatele de laborator ale pacientului.</p>
      </div>

      <div v-if="loading" class="info-box">Se încarcă rezultatele de laborator...</div>
      <div v-else-if="errorMessage" class="info-box error">{{ errorMessage }}</div>
      <div v-else-if="results.length === 0" class="info-box">Nu există rezultate de laborator pentru acest pacient.</div>
      ...
      <div v-else class="results-list">
        <div v-for="result in results" :key="result.id" class="result-card">
          ...
          <div class="result-actions">
            ...
            <button class="ai-btn" :disabled="aiLoading" @click="handleGenerateAiDiagnosis(result)">
      ...
      <div class="form-actions">
        <button class="back-btn" @click="goBack">← Înapoi la pacienți</button>
      </div>
    </div>
...
.pdf-btn { border: none; border-radius: 12px; padding: 0.9rem 1.2rem; font-size: 1rem; cursor: pointer; background: linear-gradient(135deg, #be185d, #9d174d); color: white; font-weight: 700; transition: all 0.2s; box-shadow: 0 8px 18px rgba(190,24,93,0.18); }
...
.ai-btn { border: none; border-radius: 14px; padding: 0.9rem 1.2rem; font-size: 1rem; cursor: pointer; background: linear-gradient(135deg, #7c3aed, #5b21b6); color: white; font-weight: 700; transition: all 0.2s; box-shadow: 0 8px 18px rgba(91,33,182,0.18); }
.ai-btn:hover { transform: translateY(-2px); box-shadow: 0 12px 24px rgba(91,33,182,0.25); }
.ai-btn:disabled { opacity: 0.7; cursor: not-allowed; transform: none; box-shadow: none; }

.form-actions { display: flex; justify-content: flex-start; margin-top: 1.5rem; }
.back-btn { border: none; border-radius: 14px; padding: 0.9rem 1.2rem; font-size: 1rem; cursor: pointer; background: #f3f4f6; color: #374151; font-weight: 600; }
